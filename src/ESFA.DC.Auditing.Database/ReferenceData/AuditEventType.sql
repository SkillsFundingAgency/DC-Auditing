...
USING (VALUES
	...
	WHEN MATCHED 
			AND EXISTS 
				(		SELECT Target.[EventDescription]
							  ,Target.[EventTitle]
					EXCEPT 
						SELECT Source.[EventDescription]
						      ,Source.[EventTitle]
				)
		  THEN UPDATE SET Target.[EventDescription] = Source.[EventDescription],Target.[EventTitle] = Source.[EventTitle]
